...
impl Network {
    ...
    fn backward_propagation(&mut self, cost_derivative: Tensor) {
    ...
    fn cost(output: &Tensor, target: &Tensor) -> f32 {
        let n = target.shape()[0];
        -Tensor::from_fn(vec![n], |indices| target[indices.clone()] * output[indices].ln()).sum()
    }

    fn grad_cost(output: &Tensor, target: &Tensor) -> Tensor {
        let n = target.shape()[0];
        -Tensor::from_fn(vec![n], |indices| target[indices.clone()] / output[indices])
    }

    fn validate(&mut self, validation_inputs: &Vec<Tensor>, validation_outputs: &Vec<Tensor>) -> f32 {
    ...
    pub fn train(&mut self, train_inputs: &Vec<Tensor>, train_outputs: &Vec<Tensor>, valid_inputs: &Vec<Tensor>, valid_outputs: &Vec<Tensor>, epochs: usize) {
    ...
    pub fn test_accuracy(&mut self, test_inputs: &Vec<Tensor>, test_outputs: &Vec<Tensor>) -> f32 {
